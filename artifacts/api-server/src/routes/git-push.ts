...
router.post("/git-push", (_req, res) => {
  ...
  try {
    ...
    try { execSync('git commit -m "Fix: pre-built frontend in git, API serves static files, simplified bat"', { cwd }); } catch {}
    ...
    const out = execSync("git push _gh_tmp main --force 2>&1", { cwd, timeout: 120000 }).toString();
    try { execSync("git remote remove _gh_tmp", { cwd }); } catch {}
